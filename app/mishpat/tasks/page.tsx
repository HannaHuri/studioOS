...
import {
  ...
  Calendar, ExternalLink, Check, Key, ListChecks, Maximize2, Pencil, X, Rows3, LayoutGrid, Paperclip, SlidersHorizontal,
...
export default function MishpatPage() {
  // Two top-level screens share the rail: the chat (with its documents panel) and the tasks list.
  ...
  const tasksMode = view === "tasks";
  // On the tasks screen the chat has no column of its own: it exists only as the floating companion of an open
  // document, and it starts scoped to that document.
  const chatFloating = tasksMode ? !!openDoc : (forceFloat || (!!openDoc && chatSpace < 420));
  ...
  const closeDoc = () => { setOpenDoc(null); setActiveTask(null); setDocExpanded(false); setReadingDocW(null); setChatCollapsed(true); setChatPos(null); setScopeMode("selection"); };
  // Back to the full-width task list: drop the document, keep the list exactly where it was.
  const closeTask = () => { setOpenDoc(null); setActiveTask(null); setChatCollapsed(true); setChatPos(null); setScopeMode("selection"); };
  // The rail's tasks button. Entering the tasks screen closes the documents panel — they compete for the same space
  // and mean different things; leaving it open would show the documents of one case behind a list spanning ten.
  const openTasks = () => { setView("tasks"); setIsPanelOpen(false); setFocusDocs(false); setOpenDoc(null); setActiveTask(null); setDocExpanded(false); setChatCollapsed(true); setChatPos(null); setScopeMode("selection"); };
  const openChatView = () => { setView("chat"); setOpenDoc(null); setActiveTask(null); setChatCollapsed(true); setChatPos(null); setScopeMode("selection"); };
  ...
  const resetAll = () => { setConvKey((k) => k + 1); setView("chat"); setActiveTask(null); setIsPanelOpen(false); setFocusDocs(false); setOpenDoc(null); setDocExpanded(false); setReadingDocW(null); setChatPos(null); setChatCollapsed(true); setScopeMode("selection"); };
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-hidden" style={{ backgroundColor: isDark ? dk.bg : "white" }}>
      ...
      <div ref={layoutRef} className="absolute top-12 bottom-0 left-0 right-0 flex" dir="ltr">
        {/* Chat — in-flow column normally; a draggable, resizable floating window over the document when there's no
            room for all three. On the tasks screen it exists only as the companion of an open document. */}
        {(!tasksMode || openDoc) && (
        <div
        ...
        {tasksMode && (
          <div
            className={openDoc ? "flex-shrink-0 relative" : "flex-1 min-w-0 relative"}
            style={openDoc ? { width: `${Math.min(tasksWidth, Math.max(360, vw - 60 - 480))}px` } : undefined}
          >
            <TasksView
              isDark={isDark}
              docs={docs}
            ...
            {openDoc && (
              <div
                onMouseDown={(e) => {
                  e.preventDefault();
                  setResizing(true);
                  const onMove = (ev: MouseEvent) => setTasksWidth(Math.min(vw - 60 - 480, Math.max(360, window.innerWidth - 60 - ev.clientX)));
                  const onUp = () => { setResizing(false); document.removeEventListener("mousemove", onMove); document.removeEventListener("mouseup", onUp); document.body.style.userSelect = ""; };
                  document.addEventListener("mousemove", onMove); document.addEventListener("mouseup", onUp);
                  document.body.style.userSelect = "none";
                }}
        ...
        {!tasksMode && openDoc && readingMode && !isPanelOpen && <div className="flex-1" />}
        ...
        <div className="w-[60px] flex-shrink-0 flex flex-col items-center pt-5 pb-4" style={{ borderInlineStart: `1px solid ${isDark ? dk.border : "#ebf3ff"}`, backgroundColor: sidebarBg }}>
          <button
            onClick={() => { if (tasksMode) openChatView(); newChat(); }}
          ...
          <button
            onClick={() => {
              if (tasksMode) { openChatView(); setIsPanelOpen(true); setFocusDocs(false); return; }
              if (isPanelOpen) closePanel(); else { setIsPanelOpen(true); setFocusDocs(false); }
            }}
            className="size-8 flex items-center justify-center rounded hover:bg-black/5 transition-colors"
            style={{ color: isPanelOpen && !tasksMode ? c.primary : iconCol }}
            title="מסמכים"
          >
            <Files size={19} />
          </button>
          ...
          <div className="flex-1" />
          {/* Tasks — the judge's own work list, kept at the bottom of the rail (the case-scoped tools live at the top).
              The count is the point of the button: it is the number a judge checks without opening anything. */}
          <button
            onClick={() => { if (tasksMode) openChatView(); else openTasks(); }}
            className="relative size-8 flex items-center justify-center rounded hover:bg-black/5 transition-colors mb-1"
            style={{ color: tasksMode ? c.primary : iconCol }}
            title={`משימות (${TASKS.length})`}
          >
            <ListChecks size={19} />
            <span
              className="absolute flex items-center justify-center rounded-full text-[9px] font-medium"
              style={{ top: "-1px", insetInlineStart: "-2px", minWidth: "15px", height: "15px", padding: "0 3px", backgroundColor: tasksMode ? c.primary : "#d83a52", color: "white" }}
              dir="ltr"
            >
              {TASKS.length}
            </span>
          </button>
          <div className="w-8 border-t mb-2" style={{ borderColor: isDark ? dk.border : c.border }} />
