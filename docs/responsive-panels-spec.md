...
| `CHAT_ONLY` | **760px** | מתחת אליו אף פאנל לא מוצג — נשארת רק ההתכתבות |
...
| **< 760px** | הפאנלים מוסתרים, רק ההתכתבות; שדה הכתיבה ממשיך להצטמצם | 🔴 צ'אט בלבד |
...
## 3. כוונה מול תצוגה (החלק החשוב!)

יש להפריד בין **מה שהמשתמש רוצה פתוח** (כוונה) לבין **מה שמוצג בפועל** (לפי המקום):

- `docsWanted`, `historyWanted` — כוונת המשתמש (נשמרת תמיד)
- `lastOpened` — איזה פאנל נפתח לאחרונה (`"docs"` / `"history"`)
- `showDocs`, `showHistory` — **נגזרים** מהכוונה + רוחב החלון

**כללי הנגזרת:**
1. `chatOnly` (חלון צר) → לא מציגים כלום (אבל **לא** מאפסים את הכוונה).
2. `canBoth` (חלון רחב) → מציגים כל פאנל ש"רוצים" אותו.
3. אזור בלעדי + שניהם "רצויים" → מציגים רק את **האחרון שנפתח**; השני נשאר "רצוי" ומוסתר.

**התוצאות (עונה על ההתלבטויות):**
- ❓ *"תמיד נסגרת ההיסטוריה?"* → לא. באזור הבלעדי נשארת האחרונה שנפתחה.
- ❓ *"אם מרחיבים חזרה, הפאנלים נפתחים מחדש?"* → כן. הכוונה נשמרה, אז הרחבת החלון
  **משחזרת** אוטומטית את מה שהיה פתוח. רק סגירה *יזומה* של המשתמש מאפסת את הכוונה.
- ✅ *"שתיהן פתוחות עד שלב מסוים והקלט מצטמצם"* → בדיוק כך באזור `canBoth`.
